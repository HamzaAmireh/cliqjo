...
const settings = getSetting('sugarbyte_cliq_data', {});
...
const Content = (props) => {
	...
	useEffect(() => {
		const unsubscribe = onPaymentSetup(() => {
			const cliqAliasInput = document.getElementById('sugarbyte_cliq_alias');
			const cliqAlias = cliqAliasInput ? cliqAliasInput.value : '';
			
			if (!cliqAlias.trim()) {
				return {
					type: emitResponse.responseTypes.ERROR,
					message: __('Please enter your CliQ Alias.', 'sugarbyte-mobile-bank-payments'),
				};
			}
			...
			return {
				...
				meta: {
					paymentMethodData: {
						sugarbyte_cliq_alias: cliqAlias,
					},
				},
			};
		});
	...
	return createElement(
		'div',
		{ className: 'sugarbyte-cliq-payment-block-fields', style: { padding: '16px', background: '#f7f7f7', borderRadius: '4px', marginTop: '10px' } },
		...
		createElement(
			...
			createElement('input', {
				type: 'text',
				id: 'sugarbyte_cliq_alias',
				name: 'sugarbyte_cliq_alias',
				placeholder: __('Your CliQ Alias', 'sugarbyte-mobile-bank-payments'),
				required: true,
